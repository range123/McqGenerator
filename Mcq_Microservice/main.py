...
tags_metadata = [
    ...
    {
        "name": "Ping",
        "description": "To check if the API is running ",
    }
]
app = FastAPI(
    title="MCQ Microservice",
    description="Microservice for MCQ generation from text",
    version="0.0.1",
)
...
async def generate_mcqs(inp: InputModel):
    res = qmodel.generate_question_answer(
        inp.source, max_questions=inp.max_questions)
    for i, mcq in enumerate(res):
        distractors, ans = dmodel.generate_distractors(
            mcq['question'], mcq['answer'], limit=inp.max_distractors)
        res[i]['distractors'] = distractors
    return res


@app.get('/ping', tags=['Ping'], response_model=bool)
async def ping():
    return True
