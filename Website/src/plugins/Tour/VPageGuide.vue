...
export default defineComponent({
  ...
  methods: {
    ...
    setGuides() {
      this.elements.forEach((element: any) => {
        if (this.value) {
          ...
          for (const property in this.elementDisplay) {
            element.el.style[property] = this.elementDisplay[property];
          }
          element.popper = new Popper(
            element.el,
            (this.$refs as any)[`vpageguide${element.id}`],
     ...
     */
    getPropValue(element: any, propName: string) {
      let propValue = "";
...
.v-page-guide__fade-enter-active,
.v-page-guide__fade-leave-active {
  transition: opacity 0.1s ease-out;
}
.v-page-guide__fade-enter,
.v-page-guide__fade-leave-to {
  opacity: 0;
}
</style>
